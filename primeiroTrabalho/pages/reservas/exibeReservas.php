...
<html lang='pt-br'>
    ...
    <body>
        ...
        <h1 style="margin: 20px 10px 10px 10px; text-align: center; color: gray;">Registro de Reservas: </h1>

        <?php

            echo "
            <div style='width: 50%; margin-left: 29%; padding: 10px; text-align: left;'>
                <form method='post' action='excluirReservas.php'>
                    <input type='text' name='idDigitado' placeholder='ID DA RESERVA' autofocus required style='width: 30%; padding: 10px; float: left; border: 1px solid lightgray; border-radius: 5px;'>
                ...
                <form method='post' action='editarReservas.php' style='margin-top: 10px;'>
                    <input type='text' name='idDigitado' placeholder='ID DA RESERVA' autofocus required style='width: 30%; padding: 10px; float: left; border: 1px solid lightgray; border-radius: 5px;'>
            ...
            echo "<section class='exibir'>";
                ...
                while($showRegister = mysqli_fetch_array($consult)){
                    ...
                    $id = $showRegister[0];
                    $idQuarto = $showRegister[1];
                    $idCliente = $showRegister[2];
                    $dataEntrada = $showRegister[3];
                    $dataSaida = $showRegister[4];
                    $valor = $showRegister[5];
                    ...
                    echo "<article style='margin: 10px; padding: 10px 10px 10px 40px; border-radius: 10px; background-color:  rgb(243, 174, 108); color: white;'>";
                        echo "<b>ID da reserva:</b> $id<br>";
                        echo "<b>ID do cliente:</b> $idCliente<br>";
                        echo "<b>ID do quarto:</b> $idQuarto<br>";
                        echo "<b>Entrada:</b> $dataEntrada<br>";
                        echo "<b>Saida:</b> $dataSaida<br>";
