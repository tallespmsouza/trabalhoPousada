<?php 
    require("../../connection.php");

    $id = $_POST['idDigitado'];
    $sql = "select * from reservas where reservas.idReservation = '$id'";
    $query = mysqli_query($connection,$sql);
    $reserva = mysqli_fetch_array($query);
    
?>

<!DOCTYPE html>
<html lang='pt-br'>

    <head>
        <meta charset="utf-8">
        <title>Pousada Queijo Quente</title>
        <link rel="shortcut icon" href="../../img/logo.png">
        <link rel="stylesheet" href="../../css/index.css">
    </head>

    <body>
        <!--Criando o menu de navegação-->
        <header>
            <nav id="menu">
                <ul>
                    <a href="../../index.php"><li>Início</li></a>
                </ul>
                <ul>
                    <a href="../quartos/bedrooms.php"><li>Quartos</li></a>
                </ul>
                <ul>
                    <a href="../clientes/clients.php"><li>Clientes</li></a>
                </ul>
                <ul>
                    <a href="reservations.php"><li>Reservas</li></a>
                </ul>
            </nav>
        </header>

        <h1 style="margin: 20px 10px 10px 10px; text-align: center; color: gray;">Editar Reservas: </h1>

        <?php

            echo "
                <form method='post' action='bdReservas.php' class='formulario'>
                    $reserva[0]
                    <input name='idCliente' value='$reserva[1]' class='textoArea'>
                    <input name='idQuarto' value='$reserva[2]' class='textoArea'> 
                    <input name='dataEntrada' value='$reserva[3]' class='textoArea'>
                    <input name='dataSaida' value='$reserva[4]' class='textoArea'>
                    <input name='valor' value='$reserva[3]' class='textoArea'>
                    <input name='status' value='$reserva[4]' class='textoArea'>

                    <input type='submit' value='Salvar' class='btnCriar'>
                </form>
            ";

            mysqli_close($connection);

        ?>

        <footer id="footer">
            <div class="divFooter" style="float: left;">
                <a href="https://github.com/tallespmsouza" target="_blank">
                    <img src="../../img/github.png" class="img">
                    <spam class="spamFooter">https://github.com/tallespmsouza</spam>
                </a>             
                <img src="../../img/gmail.png" class="img">
                <spam class="spamFooter">talles.pelegrine.machado@gmail.com</spam>
            </div>

            <div class="divFooter" style="float: right; text-align: left;">                
                <spam class="spamFooter" style="text-align: center;">
                    Trabalho HTML, CSS, PHP, MySql / Sistema de Informação / 6° período / Univás<br>
                    Talles Pelegrine<br>
                </spam>            
            </div>           
        </footer>

    </body>

</html>